...
import { useMediaStore } from '~/stores/media'; // Certifique-se que esse é o caminho correto
...
function onReminderButtonClick() {
  ...
  if (loading.value || isAlreadyReminded.value) return;
  ...
  >
    <div class="relative overflow-hidden rounded-t-2xl">
      ...
      <div class="absolute top-4 right-4 z-10">
        <div
          ...
          :aria-label="isAlreadyReminded ? 'Lembrete ativo' : 'Sem lembrete ativo'"
          role="img"
        >
          <svg v-if="isAlreadyReminded" class="w-4 h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
            <path
    ...
    <div class="p-4 space-y-4">
      ...
      <button
        ...
        :disabled="!props.isAuthenticated || loading || isAlreadyReminded"
        :class="[
        ...
        :aria-label="isAlreadyReminded ? `Lembrete já ativo para ${media.title}` : `Adicionar lembrete para ${media.title}`"
      >
        <svg v-if="isAlreadyReminded" class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          <path
            d="M10 2a6 6 0 00-6 6v3.586l-.707.707A1 1 0 004 14h12a1 1 0 00.707-1.707L16 11.586V8a6 6 0 00-6-6zM10 18a3 3 0 01-3-3h6a3 3 0 01-3 3z"
          />
        </svg>
        ...
        {{ isAlreadyReminded ? 'Lembrete Ativo' : 'Adicionar Lembrete' }}
      </button>
    </div>
  </article>
</template>
