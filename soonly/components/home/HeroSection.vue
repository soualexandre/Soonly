...
<template>
  <section class="relative h-screen overflow-hidden" aria-labelledby="featured-media-title">
    <div class="absolute inset-0 w-full">
      <NuxtImg :src="`https://image.tmdb.org/t/p/original/${featuredMedia?.backdrop_path}`"
        :alt="`Capa do filme ${featuredMedia?.title}`" class="w-full h-full object-cover" sizes="100vw" format="webp"
        densities="x1 x2" loading="eager" />
    ...
    <div class="relative z-20 h-full flex items-center max-w-7xl mx-auto ">
      <div class="container w-full mx-auto px-6 md:px-12 lg:px-20">
        <div class="max-w-4xl">
          ...
          <AppButton :icon="isReminded(Number(featuredMedia?.id) ?? 0) ? 'i-heroicons-bell-alert' : 'i-heroicons-bell'"
            :variant="isReminded(Number(featuredMedia?.id) ?? 0) ? 'primary' : 'secondary'" @click="handleReminderClick"
